...
only_crc_merged_table=merged_table.loc[merged_table['Group']=='CRC']
only_crc_merged_table.reset_index(drop=True,inplace=True)
"""Get and select the wanted TimePoints"""
TimePoint=input('Enter the time points to perform on  \n'+"\n".join(Constants.TimePoints_list)+"\n")
if TimePoint=='0':
    only_crc_merged_table=only_crc_merged_table[only_crc_merged_table['TimePointNum']==0]
elif TimePoint!='All':
    raise ValueError
...
for cage, mice, exp in zip(only_crc_merged_table['CageNum'], only_crc_merged_table['MiceNum'], only_crc_merged_table['Experiment']):

    dic = {'CageNum': cage, 'MiceNum': mice, 'Experiment': exp, 'TimePointNum': Constants.target_time}
    item = SA.conditional_identification(only_crc_merged_table, dic)['tumor_load'].values
    if (len(item) == 0):
        only_crc_merged_table.drop(idx, inplace=True)
    else:
        multi_class_tumor_load.append(item[0])
    idx += 1

multi_class_tumor_load = pd.Series(multi_class_tumor_load)
tumor_median=multi_class_tumor_load.median()
binary_tumor_load=multi_class_tumor_load.apply(lambda x: 1 if x>tumor_median else 0)
...
samples_bacterial_data=only_crc_merged_table.drop(Constants.mapping_and_general_info_columns,axis=1)
samples_bacterial_data_and_identification=only_crc_merged_table.drop(Constants.mapping_and_general_info_columns_no_identification,axis=1)
...
Plot.draw_rhos_calculation_figure(only_crc_merged_table['tumor_load'],only_crc_merged_table[Constants.immune_system_features],'Correlation between immune system parameters and the target',6,save_folder='Graphs\Correlation\Others')
...
labels_dict={1:'More tumors than median',0:'Less tumors than median'}
...
if TimePoint=='All':
    ...
    p1=Plot.progress_in_time_of_column_attribute_mean(dec_data,only_crc_merged_table['TimePointNum'],binary_tumor_load,margin=0.1)
    new_plot=p1.plot()
...
dec_data_at_time_of_prediction=dec_data[only_crc_merged_table['TimePointNum']==Constants.TIME_OF_PREDICTION]
kind_of_prediction=input('Enter whether you want \n'+"\n".join(Constants.prediction_sort)+"\n")

if kind_of_prediction=='Binary':
    class_list_at_time_of_prediction=binary_tumor_load[only_crc_merged_table['TimePointNum']==Constants.TIME_OF_PREDICTION]
elif kind_of_prediction=='Multi' or kind_of_prediction=='Regression' :
    class_list_at_time_of_prediction=multi_class_tumor_load[only_crc_merged_table['TimePointNum']==Constants.TIME_OF_PREDICTION]
else:
    raise ValueError
...
if kind_of_prediction!='Regression':
    ...
    for neighbors in range(1,Constants.MAX_NIGH):
        ...
        percentage.append(Clustering.cross_validation(kf,dec_data_at_time_of_prediction,class_list_at_time_of_prediction,neigh,eval_fn_name)[1])
...
'corr_TH}\nNormalization_fn : {norm}\nTumor median : {median}\nPrediction time : {pred_time}\n'.format(TP=TimePoint,pred=kind_of_prediction,
                                                              ...
                                                              norm=normalization_fn_name,median=tumor_median,pred_time=Constants.TIME_OF_PREDICTION))
    ...
    for C in [0+p/100 for p in range(1,10000)]:
        ...
        result=Clustering.cross_validation(kf,dec_data_at_time_of_prediction,class_list_at_time_of_prediction,clf,eval_fn_name)
