...
"""In merged_table_at0 every sample has its corresponding microbiome"""

samples_results=SA.data_rearrangement(reduced_exported_features)
mapping_table=pd.read_csv(Constants.maping_page_url)
sample_at0=mapping_table[mapping_table['TimePointNum']==0]
merged_table_at0=pd.merge(samples_results,sample_at0,on='#SampleID')
"""
...
for cage, mice, exp in zip(merged_table_at0['CageNum'], merged_table_at0['MiceNum'], merged_table_at0['Experiment']):
    ...
    if (len(item) == 0):
        merged_table_at0.drop(idx, inplace=True)
    else:
...
"""
merged_table_at0.drop(Constants.columns_to_be_dropped,inplace=True,axis=1)
merged_table_at0.reset_index(drop=True,inplace=True)
SA.removeZeroCols(merged_table_at0)
sampleId=merged_table_at0['#SampleID']

"""Remove highly correlated columns"""
uncorr_data=SA.dropHighCorr(merged_table_at0,Constants.THRESHOLD)
...
dec_obj,dec_data=decompose(normalized_data,Constants.dimension_reduction_dict[dimension_fn_name],n_components=5)
...
for neighbors in range(1,Constants.MAX_NIGH):
    ...
    percentage.append(Clustering.cross_validation(kf,dec_data,class_list,neigh))
